...
table(pseq_genus@sam_data$Smoking)

# Subsetting Smokers
pseq_genus <- phyloseq::subset_samples(pseq_genus, Smoking == "Yes")
table(pseq_genus@sam_data$Smoking)
...
p1 <- ggplot(Periodontitis) +
  geom_point(aes(x = Model, y = Genus, fill=SigGenera), fill = "darkgoldenrod1", size = 3, shape=21)+
  scale_x_discrete(drop=FALSE)+ # keep ALDEX2 on
  facet_wrap(~Site, scales="free_y")+
  labs(x="", y="", title = "Periodontitis", subtitle = "Smokers")+
  MyTheme
...
p2 <- ggplot(Healthy) +
  ...
  labs(x="", y="", title = "Healthy", subtitle = "Smokers")+
  MyTheme
