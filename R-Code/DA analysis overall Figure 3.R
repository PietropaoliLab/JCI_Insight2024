...
MyTheme <-   theme_bw(12)+
  theme(axis.text = element_text(colour = "black", size = 10),
        ...
        axis.text.y = element_text(hjust = 1, vjust = 0.25, size = 6.8),
        panel.grid = element_line(size = .1, color = "gray90"),
...
P_perio <- ggplot(Periodontitis) +
                  geom_point(aes(x = Model, y = Genus, fill=SigGenera), fill = "#AB3277", size = 2.2, shape=21)+
                  scale_x_discrete(drop=FALSE)+ 
                  scale_y_discrete(expand = c(0, .85), drop = TRUE)+
                    facet_wrap(~Site, scales="free_x")+
                    labs(x="", y="")+
                  MyTheme
P_perio

P_healthy <- ggplot(Healthy) +
                geom_point(aes(x = Model, y = Genus, fill=SigGenera), fill = "#049E73", size = 2.2, shape=21)+
                scale_x_discrete(drop=FALSE)+ 
                scale_y_discrete(expand = c(0, .85), drop = TRUE)+
                facet_wrap(~Site, scales="free_x")+
                labs(x="", y="")+
...
# Combine my plots
DA_analysys <- P_healthy | P_perio
DA_analysys


ggsave(plot = DA_analysys, 
       device = cairo_pdf,
       width = 9, 
       #height = 11, 
       units = "in",
