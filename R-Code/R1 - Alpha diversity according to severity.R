...
MyProject <- "PRJNA552294"
foo <- subset_samples(wgs, BioProject == MyProject)

foo@sam_data$Extension <- ifelse(grepl("LAP", foo@sam_data$LibraryName), "Localized", "Generalized")

table(foo@sam_data$Extension, foo@sam_data$Sex)
...
alpha <- merge(data.frame(foo@sam_data), alpha)
Shannon <- merge(data.frame(foo@sam_data), Shannon)
...
p+p1+plot_annotation(title = "WGS - Alpha diversity according to severity of periodontitis (PRJNA552294)")


p3 <- ggplot(alpha, aes(x = Sex, y = observed)) + 
  geom_violin(aes(fill=Sex), alpha=.3)+
  geom_point(position = position_jitter(seed = 101, width = .20), alpha = I(.7), shape=21, fill="gray90", size=2.4)+
  geom_boxplot(width =.15, fill = "white", outlier.shape = NA)+
  labs(title = "Observed diversity", y = "Observed diversity")+
...
p4 <- ggplot(Shannon, aes(x = Sex, y = diversity_shannon)) + 
  geom_violin(aes(fill=Sex), alpha=.3)+
  geom_point(position = position_jitter(seed = 101, width = .20), alpha = I(.7), shape=21, fill="gray90", size=2.4)+
  geom_boxplot(width =.15, fill = "white", outlier.shape = NA)+
  labs(title = "Shannon diversity", y = "Shannon diversity")+
  #scale_y_continuous(breaks = c(20, 40, 60, 80, 100, 120, 140, 160, 180, 200))+
  coord_fixed(ylim = c(0,7))+
  scale_fill_manual(values = c("#F88B9D", "#8ECEFD"))+
  scale_color_manual(values = c("#F88B9D", "#8ECEFD"))+
  # facet_wrap(Extension ~ Disease)+
  MyTheme+
...
p3+p4+plot_annotation(title = "WGS - Alpha diversity (PRJNA552294)")
