...
set.seed(1024)
PerioRarefactionPlot <- get_rarecurve(obj = psm_perio, chunks=400) 

# Saving Rarefaction Plot Object for further use
# save(PerioRarefactionPlot , file = "~/Google Drive/01- UNIVERSITA/AQ Projects/Microbiome/Gender features of periodontal microbiome/Data/Perio Rarefaction Plot OBJ.Rdata")

# Loading Rarefaction Plot obj
load("~/Google Drive/01- UNIVERSITA/AQ Projects/Microbiome/Gender features of periodontal microbiome/Data/Perio Rarefaction Plot OBJ.Rdata")

df <- as.data.frame(PerioRarefactionPlot$data)
...
p <- ggplot(cdata, aes(readsNums, mean, group = Sex, color = Sex, fill = Sex))+
  ...
  geom_line(size=.75)+
  facet_grid(~Site)+
  labs(title = "Periodontitis individuals", x="Numbers of reads", y="Shannon index")+
...
pdf(file = "~/Google Drive/01- UNIVERSITA/AQ Projects/Microbiome/Gender features of periodontal microbiome/Results/Figures/Rarefaction Plot Perio.pdf",
